...
Z = [[0,0],[0,0]]
trholevels = [x for x in np.linspace(-9, 0.01, 100)]
CS3 = plt.contourf(Z, trholevels, cmap=cmocean.cm.rain_r,extend="both")
plt.clf()

Z = [[0,0],[0,0]]
vvortlevels = [x for x in [0,0.005,0.01,0.015,0.02,0.025,0.03,0.035,0.04,0.045,0.05,0.055,0.06,0.065,0.07,0.075,0.08,0.085,0.09,0.095,0.1,0.105,0.11,0.115,0.12,0.125,0.13,0.135,0.14,0.145,0.15,0.155,0.16,0.165,0.17,0.175,0.18,0.185,0.19,0.195,0.2,0.205,0.21,0.215,0.22,0.225,0.23,0.235,0.24,0.245,0.25,0.255,0.26,0.265,0.27,0.275,0.28,0.285,0.29,0.3]]
CS4 = plt.contourf(Z, vvortlevels,cmap=cmocean.cm.amp)
plt.clf()
...
fig = plt.figure(figsize=(45,60),facecolor='white')
...
for i,f in enumerate(fils):
    ...
    tr=ax[i].contourf(X,Y,trhopert,trholevels,cmap=cmocean.cm.rain_r,extend='both')
    ...
    db=ax[i].contour(X, Y, ref, [40], colors='black',linewidths=8)#,linestyles='dashed')
    skip= (slice(0, None, int(2.5/dxx)), slice(0, None, int(2.5/dxx)))

    barbs=ax[i].barbs(X[skip],Y[skip],np.mean(u[find_nearest(z,0.1):find_nearest(z,1),:,:],axis=0)[skip]*1.944,np.mean(v[find_nearest(z,0.1):find_nearest(z,1),:,:],axis=0)[skip]*1.944,color="black",length=11.5)
    arrow=ax[i].quiver(X[skip],Y[skip],np.mean(xvort[find_nearest(z,0.1):find_nearest(z,1),:,:],axis=0)[skip],np.mean(yvort[find_nearest(z,0.1):find_nearest(z,1),:,:],axis=0)[skip],color="firebrick",scale=0.35,width=0.004,headwidth=4)
...
cbar_ax = plt.axes([1.0, 0.029, 0.035, 0.961])
cbar=plt.colorbar(CS3, cax=cbar_ax, ticks=[-9,-8,-7,-6,-5,-4,-3,-2,-1,0],extend='both')
cbar.set_label('Density Potential Temperature Perturbation [K]')

plt.quiverkey(arrow, 0.33, -0.01, 0.05, r'0.05 $s^{-1}$', labelpos = 'E', coordinates='figure',fontproperties={'size': 65})

patch = matplotlib.patches.Ellipse((0.66,-0.01), 0.04,0.03,linestyle='-',linewidth=6,color='k',fill=False,transform=fig.transFigure,zorder=1000)
...
ax.axis("off")
ax.text(0.76,-0.007, '40 dBZ', verticalalignment='top', horizontalalignment='right',transform=fig.transFigure, fontsize=65)


fig.subplots_adjust(hspace=0.5, wspace=0.5)
plt.tight_layout(w_pad=0)

plt.savefig('/home/aschueth/pythonscripts/Schuethetal2021/evolution_thetarho.png', bbox_inches='tight')