...
for p,t in enumerate(df.t):
    somega[p] = (dsparcels.u[t,p].values*dsparcels.xi[t,p].values+dsparcels.v[t,p].values*dsparcels.eta[t,p].values+dsparcels.w[t,p].values*dsparcels.zeta[t,p].values)/np.sqrt(dsparcels.u[t,p].values**2+dsparcels.v[t,p].values**2+dsparcels.w[t,p].values**2)
...
somega_3d=[]
omega_3d=[]
for parcelid in np.array(df.pi[somega_mask]):
...
plt.title('3D Vorticity Production in the maximum vorticity region',fontsize=25)
...
l7=plt.plot(dsparcels.t.values[:1850],np.mean(dsparcels.z[:1850,np.array(df.pi[somega_mask])],axis=1),c='lightgray',linewidth=4,label='Parcel Altitude')
plt.fill_between(dsparcels.t.values[:1850], np.amin(dsparcels.z[:1850,np.array(df.pi[somega_mask])],axis=1), np.amax(dsparcels.z[:1850,np.array(df.pi[somega_mask])],axis=1),color='lightgray',alpha=0.3)
plt.ylim(0,4000)
...
plt.savefig(f'/home/aschueth/pythonscripts/Schuethetal2021/svort_budget.pdf',dpi=300)
...
RMSE=[]
RMSPE=[]
for i, parcelid in enumerate(np.array(df.pi)[np.logical_and(np.array(subset),somega_mask)]):
