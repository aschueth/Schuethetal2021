...
ka1_file_PPI = '/home/aschueth/pythonscripts/Schuethetal2021/Ka1180613003847lvl2.nc'
ka2_file_RHI = '/home/aschueth/pythonscripts/Schuethetal2021/Ka2180613003818lvl1.nc'
...
manual_mask= (velocity.flatten() > 40)
for i in range(0,len(range_mask[:,0])):
    range_mask[i,:]=radar_PPI.range['data']>(13400)

range_mask=range_mask.astype(bool)
total_mask = [any(t) for t in zip(range_mask.flatten(), normal_mask.flatten(), manual_mask.flatten())]
...
ka1dep=pd.read_csv('/home/aschueth/pythonscripts/Schuethetal2021/20180612_deployments_ka1.csv')
...
ax1.text(0.025, 0.985, 'a)', verticalalignment='top', horizontalalignment='left',transform=ax1.transAxes, fontsize=90)
...
for j in range(int(rhib),int(rhie)+1,10):
    ...
    if ang > 360.:
        ang=int(ang-360.)
    A,B = createCircleAroundWithRadius(klat2,klon2,13,rhib,rhie+1,0)
    C,D = getLocation(klat2,klon2,ang,13)
...
display.plot_ppi_map('vel_fix', swp_id, cmap=cmocean.cm.balance, ax=ax2, vmin=-40., vmax=40.,
                     ...
                     min_lon=minlon,max_lon=maxlon,min_lat=minlat,max_lat=maxlat,embelish=False)
ax2.text(0.025, 0.985, 'b)', verticalalignment='top', horizontalalignment='left',transform=ax2.transAxes, fontsize=90)
...
ax1.plot(klon1,klat1,marker='+', transform=ccrs.PlateCarree(),color='k',markersize=32,markeredgewidth=8,path_effects=[PathEffects.withStroke(linewidth=18,foreground='xkcd:pale yellow')], zorder=10)
ax2.plot(klon1,klat1,marker='+', transform=ccrs.PlateCarree(),color='k',markersize=32,markeredgewidth=8,path_effects=[PathEffects.withStroke(linewidth=18,foreground='xkcd:pale yellow')], zorder=10)
d1a=ax1.text(klon1-0.02, klat1, 'Ka1',transform=ccrs.PlateCarree(), zorder=10)
d1a.set_path_effects([PathEffects.withStroke(linewidth=4,foreground='xkcd:pale blue')])
d1b=ax2.text(klon1-0.02, klat1, 'Ka1',transform=ccrs.PlateCarree(), zorder=10)
...
d2a=ax1.text(klon2-0.005, klat2-0.011, 'Ka2',transform=ccrs.PlateCarree(), zorder=10)
d2a.set_path_effects([PathEffects.withStroke(linewidth=4,foreground='xkcd:pale yellow')])
d2b=ax2.text(klon2-0.005, klat2-0.011, 'Ka2',transform=ccrs.PlateCarree(), zorder=10)
d2b.set_path_effects([PathEffects.withStroke(linewidth=4,foreground='xkcd:pale yellow')])

fname = '/lustre/work/aschueth/'+'roads/cb_2017_us_county_5m.shp'
...
ox.io.save_graph_shapefile(G, filepath='/lustre/work/aschueth/'+'roads/tmp'+str(1),encoding='utf-8')
fname = '/lustre/work/aschueth/'+'roads/tmp'+str(1)+'/edges.shp'
...
shutil.rmtree('/lustre/work/aschueth/'+'roads/tmp'+str(1)+'/')

fname = '/lustre/work/aschueth/'+'roads/GPhighways.shp'
shape_feature = ShapelyFeature(Reader(fname).geometries(),ccrs.PlateCarree(), edgecolor='black')
ax1.add_feature(shape_feature, facecolor='none')
ax2.add_feature(shape_feature, facecolor='none')

fname = '/lustre/work/aschueth/'+'roads/builtupp_usa.shp'
...
namsub=[]
for i in ind:
    xsub.append(x[i]+0.023)
    ysub.append(y[i]-0.005)
...
for fi,swp_id in enumerate(range(10,15)):
    radar_RHI_full = pyart.io.read(ka2_file_RHI)
    radar_RHI=radar_RHI_full.extract_sweeps([swp_id])
    scale=0.75
    ...
    velfix=aliasfix(radar_RHI.get_field(0,'corrected_velocity'),13,nyq)
    velfixdict=np.copy(dealias_data).tolist()
    velfixdict['data']=velfix
    ...
    azimuth = radar_RHI.fixed_angle['data']
    print(azimuth)
    ...
    range_mask=np.zeros(np.shape(reflectivity))
    for i in range(0,len(range_mask[:,0])):
        range_mask[i,:]=radar_RHI.range['data']>(13100)
    ...
    currentscantime=datetime.strptime('20'+ka2_file_RHI.split("/")[-1][-19:-7], "%Y%m%d%H%M%S")
    ...
    if fi ==1:
        print(np.nanmax(vort[(dist>6000)&(dist<8000)&(height>100)&(height<1000)]))
...
#         t3 = ax3.text(0.5, 1.18, 'Radar Reflectivity [dBZ]', verticalalignment='top', horizontalalignment='center',transform=ax3.transAxes, fontsize=50)    
    ...
    ax3.text(-0.2, 0.5, 'Az: '+str(np.int(np.around(azimuth)))+r'$^{\circ}$',verticalalignment='center', horizontalalignment='left', rotation=90,transform=ax3.transAxes, fontsize=50)
    c+=1
...
ax5.text(0.07, 0.41, r'Ka2 00:38 UTC',rotation=90, verticalalignment='top', horizontalalignment='left',transform=fig.transFigure, fontsize=60, fontweight='bold')
fig.subplots_adjust(wspace=0.1)
tit=plt.suptitle(r'Ka1 1.0$^{\circ}$ PPI 06/13/18 00:38 UTC',y=0.92, fontsize=70, fontweight='bold')
# plt.tight_layout()#pad=3.0)

plt.savefig(f'/home/aschueth/pythonscripts/Schuethetal2021/0612.png')