...
for parcelid in np.array(df.pi[subset][baro_mask]):
...
plt.title('3D Vorticity Production in the Baroclinic Regime',fontsize=30)
...
l7=plt.plot(dsparcels.t.values[:1850],np.mean(dsparcels.z[:1850,np.array(df.pi[subset][baro_mask])],axis=1),c='lightgray',linewidth=4,label='Parcel Altitude')
plt.fill_between(dsparcels.t.values[:1850], np.amin(dsparcels.z[:1850,np.array(df.pi[subset][baro_mask])],axis=1), np.amax(dsparcels.z[:1850,np.array(df.pi[subset][baro_mask])],axis=1),color='lightgray',alpha=0.3)
plt.ylim(0,4000)
...
plt.savefig(f'/home/aschueth/pythonscripts/Schuethetal2021/baroclinic_budget.pdf',dpi=300)
...
RMSE=[]
RMSPE=[]
for i, parcelid in enumerate(np.array(df.pi[subset][baro_mask])):
